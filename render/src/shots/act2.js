...
export async function initAct2(ctx) {
  ...
  if (!OTHER) OTHER = new Planet(await makeOtherTextures(), { rim: '#ff9a6a', nightTint: '#1a1030', term: '#ffcf5a', lightsAmt: 2.2, cloudAmt: 0.4 });
}
...
const B3 = {
  ...
  async render(ctx, s) {
    ...
    layer2D(ctx, s.target, 'screen', (g) => {
      ...
      waterfall(g, { x: 60, y: 90, w: 700, h: 520, t, cols: 120, rows: 90, line: { on: lineOn, f0: 0.62, drift: -0.08, snr: smooth(24.44, 25.6, t) * 1.2 } });
      if (lineOn) { g.fillStyle = '#ffcf5a'; g.font = '700 22px JBM'; g.fillText('CANDIDATE  ·  NARROWBAND  ·  DRIFT −0.33 Hz/s', 70, 660); }
    });
    ...
    layer2D(ctx, s.target, 'type', (g) => {
      ...
      if (A) T.text(g, 'YOUR', 1180, 170, { f: 'hero', size: 150, sx: 0.8, alpha: A.a });
      if (B) T.text(g, 'SIGNAL', 1180, 330, { f: 'hero', size: 190, sx: 0.8, alpha: B.a, color: '#ffcf5a', glow: B.hot * 30 });
      T.karaoke(g, [w[2], w[3], w[4], w[5], w[6]].map((x) => ({ ...x, text: x.text.toLowerCase() })), t, { x: 1185, y: 420, f: 'ital', size: 64, dim: 0.25 });
    });
...
const B4 = {
  ...
  async render(ctx, s) {
    ...
    layer2D(ctx, s.target, 'type', (g) => {
      ...
      if (t > w[1].t0) T.text(g, 'BEATING', 110, 300, { f: 'six', size: 330, color: '#f4f1e8', alpha: t < w[2].t0 ? 1 : 0.22 });
      if (t > w[2].t0) T.text(g, 'BLINKING', 1810, 300, { f: 'six', size: 330, align: 'right', color: '#ffcf5a', alpha: 1 - 0.8 * kick * 0 });
...
const B6 = {
  ...
  async render(ctx, s) {
    ...
    const sep = lerp(1.6, 1.25, easeInOutCubic(k));
    ...
    layer2D(ctx, s.target, 'type', (g) => {
      ...
      if (C) T.text(g, 'FAR', 960, 520, { f: 'hero', size: 200, sx: 0.8, align: 'center', alpha: C.a, glow: C.hot * 30 });
...
const B7 = {
  ...
  async render(ctx, s) {
    ...
    layer2D(ctx, s.target, 'typeBack', (g) => {
      const runs = T.layout(g, [[{ ...w[0], f: 'hero', size: 220, sx: 0.8 }], [{ ...w[1], f: 'hero', size: 220, sx: 0.8 }], [{ ...w[2], f: 'ital', size: 200, text: 'we' }, { ...w[3], f: 'hero', size: 200, sx: 0.8 }]], { x: 90, y: 70, lead: 0.86 });
...
const B8 = {
  ...
  async render(ctx, s) {
    ...
    layer2D(ctx, s.target, 'bg', (g) => {
      ...
      const zoom = Math.exp(lerp(Math.log(900), Math.log(34), easeInOutCubic(clamp(k * 1.15))));
      ORR.draw(g, t, { zoom, focus: [13.3, 10.2], highlight: 13 + 10 * 26, speed: 1.4 });
    });
    layer2D(ctx, s.target, 'type', (g) => {
      const a = clamp((t - 39.9) / 0.08);
      T.text(g, 'ALONE?', 960, 760, { f: 'six', size: 760, align: 'center', color: '#f4f1e8', alpha: a * 0.92 });
