...
def run(start, dur, tag):
    fn=f"chunk_{tag}.mp3"
    subprocess.run(["ffmpeg","-loglevel","error","-y","-ss",str(start),"-t",str(dur),"-i","song.mp3","-ac","1","-ar","16000","-b:a","64k",fn],check=True)
    ...
    json.dump(d,open(f"turbo_{tag}.json","w"),indent=1)
    res=d.get("result",{})
    out=[]
...
chunks=[(0,60,"a"),(50,60,"b"),(100,45,"c"),(135,38,"d")]
allw={}
for s,dur,tag in chunks:
    text,words=run(s,dur,tag)
